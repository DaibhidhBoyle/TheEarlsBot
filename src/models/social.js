...
Social.prototype.bindSocial = function () {
  PubSub.subscribe(SOCIAL, (msg, data) => {
    ...
    if (message.includes('!discord')) {
      this.response = `Can't get enough of the Earl and the chat?
      Join the community at https://discordapp.com/invite/PmK33d4 There's a bunch of pet pics `;
    }
    else if (message.includes('!social')) {
      this.response = `Kenny is all over the internet! Catch him at:
